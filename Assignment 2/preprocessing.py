...
cv1_train = CountVectorizer(stop_words= stop_words, lowercase=True, ngram_range = (1,1), min_df = 0.05)
...
'''
cv2 = CountVectorizer(stop_words= stop_words, lowercase=True, ngram_range = (1,2), min_df = 0.05)
...
bigram_train_values = np.concatenate((counts_bigram_train.toarray(), np.reshape(labels, (len(labels), 1))), axis = 1)
bigram_test_values = counts_bigram_test.toarray()

def make_df(features, count_matrix, labels):
    features = np.append(features, 'label')
    values = np.concatenate((count_matrix.toarray(), np.reshape(labels, (len(labels), 1))), axis = 1)
